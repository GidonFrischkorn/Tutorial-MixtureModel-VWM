...
###############################################################################!
myDir <- here()
input <- here("data","RawData_LS2018")
...
temp <- list.files(pattern = "*RCB_WM_Aging.dat")
...
# reset working directory to project location
setwd(here())
...
if (!file.exists(here("output","fit_LS2018_mixModel.RData"))) {
  ...
  save(fit_LS2018_mixModel, 
       file = here("output","fit_LS2018_mixModel.RData"),
       compress = "xz")
