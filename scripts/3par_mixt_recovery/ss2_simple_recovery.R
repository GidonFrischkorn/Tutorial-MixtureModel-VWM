...
dat <- gen_bays_3p_data(N=2000, pmem=0.6, pnt=0.3, kappa=10, setsize=2)
...
if (!file.exists(here('output/fit_3pmixt_recovery_ss2.RData'))) {
  ...
  bf_mixture1 <- bf(y ~ 1,
                    ...
                    mu3 ~ 1, # fixed intercept constrained using priors
  ...
  mix_priors1 <- 
    ...
    prior(constant(0), class = Intercept, dpar = "mu3") +
    # next, we set the guessing distribution to be uniform, kappa -> 0
    prior(constant(-100), class = Intercept, dpar = "kappa3") +
  ...
  save(list=ls(), file=here('output/fit_3pmixt_recovery_ss2.RData'))
} else {
  load(here('output/fit_3pmixt_recovery_ss2.RData'))
}
