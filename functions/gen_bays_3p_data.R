#' @title Generate artificial data from the Bays et al (2009) 3-parameter mixture model
#' @description Given a set of parameters, and desired setsize, it generates responses
#' in a continuous reproduction task, with corresponding proportions coming from memory,
#' non-target location errors, or guessing.
#' 
#' @param N number of samples
#' @param pmem probability of response coming from memory
#' @param pnt probability of response coming from a non-target item
#' @param kappa precision of the von mises distribution
#' @param setsize number of presented items
#' @param relative_resp if TRUE, returns response error relative to the target and all
#' non-target item locations are coded relative to the target. If FALSE, returns actual response
#' and the location of all items is absolute
#' 
#' @author Ven Popov

gen_bays_3p_data <- function(N=2000, pmem=0.6, pnt=0.3, kappa=10, setsize=2, relative_resp=T) {
  ...
  dat <- bind_cols(dat, as.data.frame(locations))
